...
class FindingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Nonconformity::create([
            'uuid' => Str::uuid(),
            'document_number' => 'GMP - ' . $department->abbrivation . ' - ' . Carbon::parse($request->tanggalTemuan)->format('YmdHis') . ' - ' . $id,
    ...
    public function fetchDataModal($uuid)
    {
        $nonconformity = Nonconformity::with('department')
            ->where('uuid', $uuid)
            ->firstOrFail();
